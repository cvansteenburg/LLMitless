...
async def summarize_from_disk(
    ...
    core_prompt: Annotated[
        str,
        Field(
            title="core prompt",
            description=(
                "Provides a role, context, and instructions for the LLM. LLM will"
                " summarize the content using this prompt. This is a string with"
                " brackets around template variables. At a minimum, prompt should"
                " include page_content in brackets like this: {page_content} where the"
                " contents of each summarized document will be inserted."
            ),
        ),
    ] = None,
    api_key: Annotated[
        ...
        Header(
            title="API key",
            description="Pass an API key for the summarization LLM. OpenAI is default",
        ),
    ] = None,
    ...
    collapse_prompt: Annotated[
        str | None,
        Field(
            title="collapse prompt",
            description=(
                "In a map-reduce summarization strategy, the LLM will summarize the"
                " summaries to reduce the total amount of text. LLM will use this"
                " prompt to summarize the summaries. Use template variables"
                " {core_prompt} to include the core prompt, and {context} wherever the"
                " summaries to summarize will be inserted."
            ),
        ),
    ] = None,
    combine_prompt: Annotated[
        ...
        Field(
            ...
            description=(
                "In a map-reduce summarization strategy, the last step is for the LLM"
                " to combine the summaries. LLM will use this prompt to combine the"
                " summaries. Use template variables {core_prompt} to include the core"
                " prompt, and {context} wherever the list of summaries to combine will"
                " be inserted."
            ),
        ),
    ] = None,
    *,
    max_concurrency: Annotated[
        ...
        Field(
            ...
            description=(
                "Maximum number of parallel calls to the LLM the summarizer is allowed"
                " to make."
            ),
        ),
    ] = 3,
    max_tokens_per_doc: Annotated[
        int,
        Field(
            title="Max tokens per doc",
            description=(
                "The maximum number of tokens to include in each doc that the LLM will"
                " summarize. Change this to according the context window of the LLM and"
                " the length of the prompt."
            ),
        ),
    ] = 3000,
    metadata_to_include: Annotated[
        list[str],
        Field(
            title="Metadata passed to LLM",
            description=(
                "In a map-reduce summarization strategy, docs are combined and"
                " presented together to the llm. The metadata keys are included in the"
                " combined documents to give the LLM more context."
            ),
        ),
    ] = None,
    ...
    collapse_token_max: Annotated[
        int,
        Field(
            title="Collapse token max",
            description=(
                "In a map-reduce summarization strategy, this is the maximum number of"
                " tokens to include in the combined summaries that the LLM will"
                " summarize."
            ),
        ),
    ] = 6000,
